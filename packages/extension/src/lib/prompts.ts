...
- Keep execute-js code simple. Target one element per action. Use querySelector with specific selectors.`

function buildCapabilityRules(caps: Capabilities): string {
  const rules: string[] = []

  if (caps.navigate !== false) {
    rules.push('- "navigate": send the user to a specific page. Set "target" to the URL path.')
  }
...
export function buildSystemPrompt(mode: 'manifest' | 'no-manifest', caps: Capabilities): string {
  const intro =
    mode === 'manifest'
...
export function buildUserPrompt(opts: {
  ...
  if (opts.pageContext) {
    parts.push(`<current-page-elements>\n${opts.pageContext}\n</current-page-elements>`)
  }
