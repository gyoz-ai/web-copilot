...
export default defineBackground({
  ...
  main() {
    ...
    // Safari 18 bug: cookies.get returns null unless storeId is passed explicitly.
    // Workaround: iterate cookie stores from getAllCookieStores().
    console.log(
      "[gyoza:cookie] Startup: checking for existing session cookie...",
    );
    (async () => {
      try {
        let cookie: Browser.cookies.Cookie | null = null;

        // Try default call first (works on Chrome/Firefox)
        cookie = await browser.cookies.get({
          url: `https://${PLATFORM_DOMAIN}`,
          name: SESSION_COOKIE,
        });

        // Safari 18 workaround: iterate cookie stores with explicit storeId
        if (!cookie && browser.cookies.getAllCookieStores) {
          const stores = await browser.cookies.getAllCookieStores();
          console.log(
            "[gyoza:cookie] Trying storeId workaround, stores:",
          ...
          for (const store of stores) {
            ...
            if (cookie) {
              console.log("[gyoza:cookie] Found via storeId:", store.id);
              break;
            }
          }
        }
        ...
        console.log(
          "[gyoza:cookie] Startup result:",
          cookie ? `found (${cookie.domain})` : "not found",
        );
        if (!cookie) return;
        ...
        if (settings.managedToken === cookie.value && settings.managedTier)
          return;
        console.log("[gyoza:cookie] Startup: syncing session cookie");
        let managedPlan: string | undefined;
        let managedTier: string | undefined;
        try {
          const res = await fetch(`${PLATFORM_URL}/v1/ai/usage`, {
        ...
        console.warn("[gyoza:cookie] Startup check failed:", err);
      }
    })();
    ...
    browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      switch (message.type) {
        ...
        case "gyozai_capture_tab":
          browser.tabs
            .captureVisibleTab(null as unknown as number, {
              format: "jpeg",
              quality: 70,
            })
            .then((dataUrl) => {
            ...
            .catch((err) => {
              console.error("[gyoza] capture_tab failed:", err);
              sendResponse({
