...
export function capturePageContext(types: SnapshotType[] = ['all']): PageContext {
  ...
  if (captureAll || types.includes('textContent')) {
    ...
    const SKIP_TAGS = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT', 'SVG', 'ASTRO-ISLAND'])
...
export function formatPageContext(ctx: PageContext): string {
  ...
  if (ctx.headings.length > 0) {
    ...
    ctx.headings.forEach((h) => parts.push(`  <h${h.level}>${h.text}</h${h.level}>`))
    parts.push('</page-headings>')
  }

  if (ctx.buttons.length > 0) {
    parts.push('<page-buttons>')
    ctx.buttons.forEach((b) => parts.push(`  <button selector="${b.selector}">${b.text}</button>`))
    parts.push('</page-buttons>')
  }
  ...
  if (ctx.links.length > 0) {
    ...
    ctx.links.forEach((l) =>
      parts.push(`  <link href="${l.href}" selector="${l.selector}">${l.text}</link>`),
    )
    parts.push('</page-links>')
  }
  ...
  if (ctx.inputs.length > 0) {
    ...
    ctx.inputs.forEach((inp) => {
      const attrs = [`name="${inp.name}"`, `type="${inp.type}"`, `selector="${inp.selector}"`]
