...
describe('engine.query', () => {
  test('sends POST to proxy and dispatches navigate action', async () => {
    ...
    mockFetch({
      actions: [{ type: 'navigate', target: '/dairy', message: 'Going to dairy.' }],
    })
  ...
  test('sends correct payload to proxy', async () => {
    ...
    globalThis.fetch = mock((url: string, init: RequestInit) => {
      ...
      return Promise.resolve(
        new Response(JSON.stringify({ actions: [{ type: 'show-message', message: 'ok' }] }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        }),
      )
    ...
    expect(body.sitemapXml).toBe('<test-sitemap />')
  ...
  test('dispatches click action', async () => {
    ...
    mockFetch({
      actions: [{ type: 'click', selector: '#buy-btn', message: 'Clicking buy.' }],
    })
  ...
  test('maintains conversation history across queries', async () => {
    ...
    expect(engine.getHistory()[2]).toEqual({ role: 'user', content: 'now bakery' })
  })
    ...
    globalThis.fetch = mock(() => {
      ...
      const response =
        callCount === 1
          ...
          : { actions: [{ type: 'navigate', target: '/product/mancini', message: 'Found it!' }] }
...
describe('clarify action', () => {
  test('calls onClarify with message and options', async () => {
    ...
    const engine = createEngine({
      proxyUrl: 'http://localhost:3001',
      onClarify: (msg, opts) => clarifications.push({ message: msg, options: opts }),
    })
...
describe('error handling', () => {
  test('calls onError on network failure', async () => {
    ...
    await expect(engine.query('test')).rejects.toMatchObject({ type: 'network' })
  ...
  test('calls onError on proxy error (4xx/5xx)', async () => {
    ...
    await expect(engine.query('test')).rejects.toMatchObject({ type: 'proxy', status: 429 })
  ...
  test('calls onError on invalid response', async () => {
    ...
    await expect(engine.query('test')).rejects.toMatchObject({ type: 'validation' })
