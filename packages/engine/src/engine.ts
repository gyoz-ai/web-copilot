...
import { capturePageContext, formatPageContext, type SnapshotType } from './page-context'
...
export function createEngine(config: EngineConfig) {
  ...
  async function query(text: string, opts?: QueryOptions): Promise<QueryResult> {
    ...
    if (typeof sessionStorage !== 'undefined') {
      const pendingSnapshots = sessionStorage.getItem('gyozai_pending_snapshots')
      if (pendingSnapshots) {
    ...
    if (manifestMode && config.sitemapXml) {
      payload.sitemapXml = config.sitemapXml
    }
    ...
    if (!response.ok) {
      const body = await response.json().catch(() => ({ error: 'Unknown proxy error' }))
      const error: EngineError = {
        type: 'proxy',
        message: (body as { error?: string }).error || `Proxy returned ${response.status}`,
    ...
    const allowedActions = validation.data.actions.filter((a) => capabilityMap[a.type] !== false)
    const result = { actions: allowedActions.length > 0 ? allowedActions : validation.data.actions }
    ...
    const extraRequests = (validation.data as { extraRequests?: string[] }).extraRequests
    if (extraRequests && extraRequests.length > 0) {
      ...
      const hasPageChange = result.actions.some(
        (a) => a.type === 'navigate' || a.type === 'click' || a.type === 'execute-js',
      )
      ...
      if (hasPageChange) {
        ...
        if (typeof sessionStorage !== 'undefined') {
          sessionStorage.setItem('gyozai_pending_snapshots', JSON.stringify(snapshotTypes))
        }
        ...
        const hasRealActions = result.actions.some((a) => CONTENT_ACTIONS.includes(a.type))
        ...
        if (hasRealActions || autoFollowUpUsed) {
          ...
          console.log('[gyozai] Auto-follow-up: captured context, re-querying...')
          const reResult = await query(
            'Now answer my question with the page context provided.',
            opts,
          )
          autoFollowUpUsed = false
          console.log('[gyozai] Auto-follow-up result:', reResult.actions.length, 'actions')
          const originalMessages = result.actions.filter((a) => a.type === 'show-message')
    ...
    if (fetchAction && fetchAction.url && config.httpClient) {
      ...
      const fetchResult = await config.httpClient(fetchAction.url, fetchAction.method || 'GET')
      ...
      return query(`Based on the fetched results, answer my original question: ${text}`, opts)
    }
...
function dispatchAction(action: Action, config: EngineConfig): string | undefined {
    ...
    case 'highlight-ui':
      if (action.selector) {
        ...
        if (typeof document !== 'undefined') {
          const hlEl = document.querySelector(action.selector) as HTMLElement | null
          if (hlEl) {
